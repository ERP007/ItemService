...
@Schema(description = "내부 서비스용 부품 상세 조회 응답")
public record InternalItemDetailResponse(
    ...
    public static InternalItemDetailResponse from(ItemView item) {
        return new InternalItemDetailResponse(
                item.sku(),
                item.name(),
                InternalItemCategoryResponse.majorCategory(item),
                InternalItemCategoryResponse.middleCategory(item),
                item.unit().getCode(),
                item.unitPrice(),
                item.safetyStock(),
                item.active()
